...
from database import Prediction, get_db_session
...
def api_drift_check():
    ...
    try:
        ...
        this_conf = db.query(func.avg(Prediction.confidence)).filter(Prediction.timestamp >= this_week_start).scalar()

        last_conf = (
            db.query(func.avg(Prediction.confidence))
            .filter(Prediction.timestamp >= last_week_start, Prediction.timestamp < this_week_start)
            .scalar()
        )

        this_count = db.query(func.count(Prediction.id)).filter(Prediction.timestamp >= this_week_start).scalar() or 0
...
def api_predictions_over_time():
    ...
    try:
        rows = (
            ...
            .group_by("hour", Prediction.prediction)
            .order_by("hour")
            .all()
        )
...
def api_download_filtered():
    ...
    try:
        ...
        query = db.query(Prediction).filter(Prediction.timestamp >= datetime.utcnow() - timedelta(days=days))
