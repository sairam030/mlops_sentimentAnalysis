...
import yaml
from database import Prediction, get_db_session, init_db
...
def predict():
    ...
    try:
        if MODEL_TYPE == "transformers":
            ...
            predictions = [{"label": str(pred), "score": 1.0} for pred in raw_predictions]
        ...
        return jsonify({"model_type": MODEL_TYPE, "predictions": predictions, "response_time_ms": response_time})
...
def monitoring_stats():
    ...
    try:
        ...
        total = db.query(func.count(Prediction.id)).filter(Prediction.timestamp >= yesterday).scalar() or 0

        avg_confidence = db.query(func.avg(Prediction.confidence)).filter(Prediction.timestamp >= yesterday).scalar()

        label_dist = (
            db.query(Prediction.prediction, func.count(Prediction.id))
            .filter(Prediction.timestamp >= yesterday)
            .group_by(Prediction.prediction)
            .all()
        )

        avg_response_time = (
            db.query(func.avg(Prediction.response_time_ms)).filter(Prediction.timestamp >= yesterday).scalar()
        )
...
def check_drift():
    ...
    try:
        ...
        this_week_conf = db.query(func.avg(Prediction.confidence)).filter(Prediction.timestamp >= this_week).scalar()

        last_week_conf = (
            db.query(func.avg(Prediction.confidence))
            .filter(Prediction.timestamp >= last_week, Prediction.timestamp < this_week)
            .scalar()
        )

        this_week_count = db.query(func.count(Prediction.id)).filter(Prediction.timestamp >= this_week).scalar() or 0
...
def dashboard_stats():
    ...
    try:
        ...
        # Last 24 hours
        yesterday = datetime.utcnow() - timedelta(days=1)
        recent_24h = db.query(func.count(Prediction.id)).filter(Prediction.timestamp >= yesterday).scalar() or 0
...
def confidence_ranges():
    ...
    try:
        ...
        for label in ["positive", "negative", "neutral"]:
            fig.add_trace(go.Bar(name=label.title(), x=ranges, y=data[label], marker_color=colors[label]))
...
def predictions_over_time():
    ...
    try:
        ...
        daily_counts = (
            ...
            .group_by("date", Prediction.prediction)
            .order_by("date")
            .all()
        )
